...
export class MonitoringOfWorksComponent implements OnInit {
  ...
  onEnter(valueModi: string) {
    ...
    this.http.post('http://20.127.19.239/company/getOffreStep3ById', queryObj)
      .subscribe(res => {
        for ([key, val] of Object.entries(res)) {
  ...
  addToOffre() {
    ...
    if (!this.form.valid) {
      ...
      this.http.post('http://20.127.19.239/company/Step3Offre', queryObj)
        .subscribe(res => {
          for ([key, val] of Object.entries(res)) {
            if (key == "resutFunction") {
              if (val == 1) {
  ...
  deleteAnWorkerFromOffre(workerProfile: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.http.post('http://20.127.19.239/company/deleteAnWorkerFromOffreStep3', queryObj)
          .subscribe(res => {
            for ([key, val] of Object.entries(res)) {
              if (key == "resutFunction") {
                if (val == 1) {
  ...
  updateInformationClientTODB() {
    ...
    this.http.post('http://20.127.19.239/company/updateInformationOfStep3', queryObj)
      .subscribe(res => {
        for ([key, val] of Object.entries(res)) {
          this.visible3 = !this.visible3
          this.onEnter(this.idOffreSendGlobale)
