...
export class ApprovalOfExecutionFilesComponent implements OnInit {
  ...
  onEnter(valueModi: string) {
    ...
    this.http.post('http://localhost:5050/company/getOffreStep2ById', queryObj)
      .subscribe(res => {
        for ([key, val] of Object.entries(res)) {
  ...
  addToOffre() {
    ...
    if (!this.form.valid) {
      ...
      this.http.post('http://localhost:5050/company/Step2Offre', queryObj)
        .subscribe(res => {
          for ([key, val] of Object.entries(res)) {
            if (key == "resutFunction") {
              if (val == 1) {
  ...
  deleteAnWorkerFromOffre(workerProfile: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.http.post('http://localhost:5050/company/deleteAnWorkerFromOffreStep2', queryObj)
          .subscribe(res => {
            for ([key, val] of Object.entries(res)) {
              if (key == "resutFunction") {
                if (val == 1) {
  ...
  updateInformationClientTODB() {
    ...
    this.http.post('http://localhost:5050/company/updateInformationOfStep2', queryObj)
      .subscribe(res => {
        for ([key, val] of Object.entries(res)) {
          this.visible3 = !this.visible3
          this.onEnter(this.idOffreSendGlobale)
