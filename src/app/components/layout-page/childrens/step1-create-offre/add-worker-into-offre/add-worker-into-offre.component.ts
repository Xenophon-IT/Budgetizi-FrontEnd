...
export class AddWorkerIntoOffreComponent implements OnInit {
  ...
  onEnter(valueModi: string) {
    ...
    this.http.post('http://20.127.19.239/company/getOffreById', queryObj)
      .subscribe(res => {
        for ([key, val] of Object.entries(res)) {
  ...
  addToOffre() {
    ...
    if (!this.form.valid) {
      ...
      this.http.post('http://20.127.19.239/company/Step1Offre', queryObj)
        .subscribe(res => {
          for ([key, val] of Object.entries(res)) {
            if (key == "resutFunction") {
              if (val == 1) {
  ...
  updateInformationClientTODB() {
    ...
    this.http.post('http://20.127.19.239/company/updateInformationOfStep1', queryObj)
      .subscribe(res => {
        for ([key, val] of Object.entries(res)) {
          this.visible3 = !this.visible3
  ...
  deleteAnWorkerFromOffre(workerProfile: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.http.post('http://20.127.19.239/company/deleteAnWorkerFromOffre', queryObj)
          .subscribe(res => {
            for ([key, val] of Object.entries(res)) {
              this.visible3 = !this.visible3
              this.onEnter(this.idOffreSendGlobale)
