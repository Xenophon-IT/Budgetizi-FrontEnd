...
export class ArchivesOffresComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private fb: FormBuilder) {
    ...
    this.http.post('http://localhost:5050/company/archivesOffres', queryObj)
      .subscribe(res => {
    ...
    // this.http.post('http://localhost:5050/company/getOffreFromArchives', queryObj2)
    //   .subscribe(res => {
    //     for ([key, val] of Object.entries(res)) {
    //       if (key == "resutFunction") {
    //         console.log(val)
  ...
  editAnOffre(idOffre: any) {
    ...
    console.log(idOffreSend)
    this.http.post('http://localhost:5050/company/getOffreFromArchives', queryObj)
      .subscribe(res => {
        for ([key, val] of Object.entries(res)) {
          if (key == "resutFunction") {
            console.log(val)
  ...
  updateInformationOffreNegociation() {
    if (!this.form.valid) {
      ...
      this.http.post('http://localhost:5050/company/updateInformationOffreNegociation', queryObj)
        .subscribe(res => {
          for ([key, val] of Object.entries(res)) {
            if (key == "resutFunction") {
              var key2, val2;
